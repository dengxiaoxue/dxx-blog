...
const initTheme = (t: any) => {
  ...
  // 每个模块的theme组成的数组
  const moduleThemeArr = Object.values(t.theme)
    .map((e: any) => e.default)
    .filter((e) => !!e)
    .flat()
  // 分类收集主题色下的css属性
  for (const themeItem of moduleThemeArr) {
    for (const key in themeItem) {
      ...
      if (
        (typeof value == 'string' && (themeMap.light[key] = value),
        typeof value == 'object')
      ) {
...
const setTheme = (t: any) => {
  document.body.classList.add(`${prefix}light`)
  t === 'light' &&
    document.body.classList.remove(`${prefix}${currentTheme.value}`)
...
const $ = (t: any) =>
  computed(() => {
    var n
    return (
      ((n = themeMap == null ? void 0 : themeMap[currentTheme.value]) == null
        ? void 0
        : n[t]) ?? t
    )
  })
const vStyle = (t: any, n: any) => {
  ...
  return (
    ...
    new Promise((s, o) => {
      ...
      let u = ''
      for (const f in n) u += `${f}:${n[f]};`
      r.appendChild(document.createTextNode(`.${t}{ ${u} }`)),
        document.getElementsByTagName('head')[0].appendChild(r),
        (r.onload = () => s('load success')),
        (r.onerror = () => o('load error'))
    })
  )
}
let languageMap: any = {
...
const initLang = (t: any) => {
  ...
  for (const e of n)
    for (const s in e) {
      const o = e[s]
      if (
        (typeof o == 'string' && (languageMap['zh-CN'][s] = o),
        typeof o == 'object')
      )
        for (const r in o) languageMap[r][s] = o[r]
    }
}
const setLang = (t: any) => {
  currentLang.value = t
...
const i18n = (t: any) =>
  computed(() => {
    var n
    return (
      ((n = languageMap == null ? void 0 : languageMap[currentLang.value]) ==
      null
        ? void 0
        : n[t]) ?? t
    )
...
const dasWeb = {
  install: (t: any, n: any) => {
    t.use(L, n), // 安装自定义插件
      ...
      n.router
        ? {
            routes: Object.values(n.router)
              .map((s: any) => s.default)
              .filter((s) => !!s)
              .flat()
              .sort((s, o) => (s.index ?? 0) - (o.index ?? 0))
          }
        : { routes: [] }
  },
  // 以下这些看起来也没什么用
...
const L = {
  install: (t: any, n: any) => {
    // 自定义插件
    n.plugins &&
      Object.values(n.plugins)
        .sort((e: any, s: any) => (e.index ?? 0) - (s.index ?? 0))
        .forEach((e: any) => {
